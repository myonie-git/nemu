...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
    ...
    INSTPAT_START();
    INSTPAT("0000?? ? ????? ????? 111 ????? 00010 11", mvl   , VSET, cpu.mvl.xlen = tmp1; cpu.mvl.ylen = tmp2; );//printf("mvl.xlen is set to: %ld, mvl.ylen is set to: %ld\n", tmp1, tmp2););
    INSTPAT("0001?? ? ????? ????? 111 ????? 00010 11", mgpr  , VSET, mgpr_x(dest) = tmp1; mgpr_y(dest) = tmp2; );//printf("mgpr[%d] x is set t0: %ld, y is set to: %ld\n",dest, tmp1, tmp2););
    ...
    INSTPAT("0101?? ? ????? ????? 111 ????? 00010 11", mctype, VSET, cpu.mctype = tmp1; );//printf("mctype is set to %ld\n", tmp1););
