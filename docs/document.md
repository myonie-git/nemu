...
忆阻器位置寄存器用于记录一组需要计算的数据在忆阻器中的位置，忆阻器寄存器长度为64位，每16位分别记录数据在忆阻器中的起始点的x坐标，y坐标以及长，宽。同时，我们设置了一系列[忆阻器位置寄存器设置指令](#353-忆阻器位置寄存器设置指令-seti)以对其进行操作。
...
### 2.6 Tradeoff between RV64V and RV64MC
...
- 寄存器的数量不会有32个之多，会有足够的指令空间留给这些指令
